...
% Notes: Attempt 1 using Poissuelle flow field as background.
%--------------------------------------------%
...
UPM = 1; % Magnitude of Poisselle flow.
...
R02 = 0.1; % Radius of the stokelet surface.
H0 = -0.5; % Initial height
stks1 = [cent1 + R01*cos(theta'),H0+R01*sin(theta');]; % Get the stokeslet cartesian coordinates.
stks2 = [cent2 + R02*cos(theta'),H0+R02*sin(theta');]; % Get the stokeslet cartesian coordinates.
stks = [stks1;stks2];
%
%%
...
for n = 1:Nstoks
    U0(:,1:2) = U0(:,1:2) - UPM*U_pois(stks(n,2),stks(n,1));
end
...
UxTemp = nullflow(2:2:end,2:2:end).*(Ux + UPM*UyPois(2:2:end,2:2:end));
UyTemp = nullflow(2:2:end,2:2:end).*(Uy + UPM*UxPois(2:2:end,2:2:end));
